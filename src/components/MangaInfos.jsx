...
return (
    <div className="w-full h-full flex flex-col items-center justify-center space-y-8">
        ...
        <img className="w-64 h-96 object-cover rounded-md shadow-lg" src={
            IfImageNotFound(mangas, mangaId)
            } alt="image" />
        <p className="text-lg">Nombre de volumes publiés : {mangas.find(manga => manga.mal_id === parseInt(mangaId)).volumes}</p>
        <p className="text-lg">Nombre de chapitres publiés : {mangas.find(manga => manga.mal_id === parseInt(mangaId)).chapters}</p>
        <p className="text-lg">{mangas.find(manga => manga.mal_id === parseInt(mangaId)).status}</p>
        <p className="text-lg">Première publication le {mangas.find(manga => manga.mal_id === parseInt(mangaId)).published.string}</p>
        <p className="text-lg">{mangas.find(manga => manga.mal_id === parseInt(mangaId)).synopsis}</p>
        <p className="text-lg">Auteur : {mangas.find(manga => manga.mal_id === parseInt(mangaId)).authors[0].name}</p>
